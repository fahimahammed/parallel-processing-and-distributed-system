...
int countPatternOccurrences(const vector<string> &words, const string &pattern)
{
    ...
    for (const auto &w : words)
    {
        ...
        {
            // %x%: 'x' is in the middle of the word
            occurrences++;
        }
        else if (pattern.size() == 2 && pattern[0] == '%' && w.find(pattern[1]) != string::npos && w.find(pattern[1]) == w.size() - 1)
        {
            // %x: 'x' is at the end of the word
            occurrences++;
        }
        else if (pattern.size() == 2 && pattern[1] == '%' && w.find(pattern[0]) != string::npos && w.find(pattern[0]) == 0)
        {
            // x%: 'x' is at the beginning of the word
            occurrences++;
        }
    }
    return occurrences;
...
int main(int argc, char **argv)
{
    ...
    if (!worldRank)
    {
        ...
        for (int i = 1; i < worldSize; i++)
        {
            ...
            // cout << "Total occarances: " << totalOccurrences << endl;
        }
    ...
    else
    {
        ...
        cout << "rank: " << worldRank << "   Occrance: " << occurrences << "\n strings:   " << segmentString << endl;
